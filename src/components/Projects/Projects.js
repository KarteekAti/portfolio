...
export default function Project() {
    ...
    return (
        <section id='projects' className='mt-10 px-6 py-3 flex flex-col justify-center items-center'>
                    ...
                    projects.map((project, i) => (
                        <div className="max-w-xs rounded bg-gray-100 dark:bg-[#111111] overflow-hidden shadow-md relative hover:-translate-y-2 transition-all ">
                            ...
                            <div className="px-6 pt-2 flex flex-wrap justify-center">
                                {project.tech.map((stack, j) => <span className="inline-block bg-purple-700 rounded-full px-3 py-1 text-sm font-semibold text-white mr-2 mb-2">{stack}</span>)}
                            </div>
                        </div>
                    ))
                }
